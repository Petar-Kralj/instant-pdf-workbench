  ...
  return (
    <label
      ...
      className={`relative flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed p-12 cursor-pointer transition-all duration-300
        ${dragging ? 'border-primary bg-primary/5 shadow-soft-hover scale-[1.01]' : 'border-border bg-card shadow-soft-inset hover:border-primary/40 hover:shadow-soft'}`}
    >
      <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center">
        <Upload className="w-6 h-6 text-primary" />
      </div>
      <div className="text-center">
        <p className="font-semibold text-sm text-foreground">{label || 'Drop files here or click to browse'}</p>
        <p className="text-xs text-muted-foreground mt-1.5">{sublabel || `Accepts ${accept} files`}</p>
      </div>
